...
re_for_file_B = re.compile(r"(\/\w+\/\w+).*");
...
def LookupListInDict(slist, sdict, mlist, dlist):
    ...
    for record in slist:
        if record.getValidData() in sdict:
            mlist.append(sdict[record.getValidData()])
        else:
            dlist.append(record)

def reportInfo(mlist, dlist):
...
def call_find(file_A, expression_A, file_B, expression_B):
    global to_dict, to_list, diff_list, match_list
    putRecordToHash(file_B, expression_B, to_dict)
    putRecordToList(file_A, expression_A, to_list)
    LookupListInDict(to_list, to_dict, match_list, diff_list)
    reportInfo(match_list, diff_list)
...
if __name__ == "__main__":
    ...
    fileA = sys.argv[1]
    fileB = sys.argv[2]
    call_find(fileA, re_for_file_A, fileB, re_for_file_B)
