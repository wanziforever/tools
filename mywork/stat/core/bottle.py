...
from common.errors import CodedError, PermissionDeniedError, AuthError
...
from core.measure import execution_duration_measure
import logging
from core.staticCount import StaticCount
...
#         self.install(compressor_plugin)
#        self.install(SignaturePlugin())
        self.install(ViewResolverPlugin())
        self.install(ExcHandlerPlugin())
        self.install(TemplatePlugin())
        self.install(TimeMeasurePlugin())
        self.install(AuthPlugin())
        self.install(MonitorPlugin())
    ...
    def init_request(self):
        request.isapi = self.isapi if hasattr(self, 'isapi') else False
        # set default view resolver
        if request.isapi:
            request.viewresolver = JSONResolver()
        else:
            request.viewresolver = HTMLResolver()
    ...
    def _cast(self, out, peek=None):
        ...
        if isinstance(out, HTTPError):
            out.apply(response)
            if hasattr(self, '_default_error_handler'):
                default_handler = self._default_error_handler
            else:
                default_handler = self.default_error_handler
            out = self.error_handler.get(out.status_code, default_handler)(out)
...
'''
a plugin to resolve output to json, xml ...
@author: shaoguojian
'''
class ViewResolverPlugin(object):
    name = 'resolver'
    api = 2
    ...
    def setup(self, app):
        self.viewresolvers = []
        self.viewresolvers.append(JSONResolver())
        self.viewresolver = None
    
    def apply(self, callback, route):
        def wrapper(*args, **kwargs):
            for viewresolver in self.viewresolvers:
                if viewresolver.resolvable():
                    self.viewresolver = viewresolver
                    break
            if self.viewresolver:
                request.viewresolver = self.viewresolver
            else:
                self.viewresolver = request.viewresolver
            rv = callback(*args, **kwargs)
            return self.viewresolver.resolve(rv)
        return wrapper
...
class ViewResolver(object):
    ...
    def resolve(self, out):
        if isinstance(out, HTTPError):
            ret = self.resolve_error(out)
            log.warn('HTTP Error found: {0}'.format(ret))
            return ret
        else:
            return self.resolve_view(out)
    
    def resolve_view(self, out):
        return out
    
    def resolve_error(self, e):
        return e.body
    

class JSONResolver(ViewResolver):
    
    def resolvable(self):
        if request.path.endswith('.json') or \
            request.query.get('format', '').lower() == 'json' or \
            request.headers.get('Accept', '').lower() == 'application/json':
            return True
        return False
    ...
    def resolve_view(self, out):
        def default_encoder(obj):
            return obj.__json__()
        if isinstance(out, dict) and out.has_key("special_content_type"):
            pass
        else:
            response.content_type = 'application/json'
        if out is None:
            out = {'success': True}
        return json_dumps(out, default=default_encoder)   
    ...
    def get_http_statuses(self):
        ...
        for k in dir(http):
            if k.startswith('_'):
                continue
            http_statuses.append((getattr(http, k, ''), k))
        return dict(http_statuses)
    
    def resolve_error(self, e):
        if isinstance(e.exception, CodedError):
            rv = e.exception.__json__()
            for k in rv.keys():
                if k.startswith('_'):
                    del rv[k]
            rv['success'] = False
            ...
            response.content_type = 'application/json'
            if not hasattr(self, 'http_statuses'):
                self.http_statuses = self.get_http_statuses()
            rv = dict(success=False, error=self.http_statuses.get(e.status_code, 'UNKNOWN_ERROR'), message=e.body)
        if DEBUG:
            try:
                rv_extra = {}
                if e.exception:
                    rv_extra['exception'] = tob(e.exception)
                if e.traceback:
                    rv_extra['traceback'] = tob(e.traceback)
                rv.update(rv_extra)
            except:
                pass
        return json_dumps(rv)

class XMLResolver(ViewResolver):
    
    def resolvable(self):
        if request.path.endswith('.xml') or \
            request.query.get('format', '').lower() == 'xml' or \
            request.headers.get('Accept', '').lower() == 'application/xml':
            return True
        return False
...
class HTMLResolver(ViewResolver):
    
    def resolvable(self):
        if request.path.endswith('.html') or \
            request.path.endswith('.htm') or \
            request.query.get('format', '').lower() in ['html', 'htm'] or \
            request.headers.get('Accept', '').lower() == 'text/html':
            return True
        return False
...
class CompressorPlugin(object):
    ...
    def client_wants_encoding(accept_encoding_header):
        ...
        """
        encodings = CompressorPlugin.parse_encoding_header(accept_encoding_header)
        ...
        if gzip_value or deflate_value:
            if deflate_value > gzip_value:
                return CompressorPlugin.Deflate_Encoding
            else:
                return CompressorPlugin.Gzip_Encoding
        elif '*' in encodings:
            return CompressorPlugin.Gzip_Encoding
        else:
            return None
    ...
    def apply(self, callback, route):
        ...
        def wrapper(*args, **kwargs):
            ...
            encoding_type = CompressorPlugin.client_wants_encoding(accept_encoding) \
            ...
            if encoding_type:
                ...
                if len(data) < minimal_size:
                    return [data]
                if encoding_type == CompressorPlugin.Gzip_Encoding:
                    data = CompressorPlugin.compress(data, compress_level)
...
'''
exception handler plugin: convert CodedError to HTTPError
'''
class ExcHandlerPlugin(object):
    name = 'exchandler'
    api = 2
    ...
    def apply(self, callback, route):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                # re-raise http error because callback may be
                # return http error instead of raising it
                if isinstance(ret, HTTPError):
                    raise ret
                return ret
            except CodedError as e:
                raise make_http_error(e)
            except Exception:
                raise
        return wrapper
    
def make_http_error(coded_error):
    if hasattr(coded_error, '_status_code'):
        return HTTPError(status=coded_error._status_code, exception=coded_error)
    else:
        return HTTPError(status=500, exception=coded_error)

class TimeMeasurePlugin(object):
    name =  'time measure'
    api = 2
    ...
    def apply(self, callback, route):
        def wrapper(*args, **kwargs):
            ...
            execution_duration_measure(before, after)
            return ret
        return wrapper
    ...
    def apply(self, callback, route):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if monitor.get_time_count() == 0:
                    monitor.set_time_count(t)
                long_time = t - monitor.get_time_count()
                if long_time > 300:
                    monitor.five_initial()
                monitor.count_add()
                date_start = datetime.now()
                start = date_start.microsecond + date_start.second * 1000 + date_start.minute * 1000000
                ret = callback(*args, **kwargs)
                date_end = datetime.now()
                end = date_end.microsecond + date_end.second * 1000 + date_end.minute * 1000000
                if start > end:
                    monitor.fail_count_add()
                    log.info('======start time > end time========')
                else:
                    process_time = end - start
                    monitor.process_count_add(process_time)
                    monitor.long_add(process_time)
                return ret    
            except Exception:
                monitor.fail_count_add()
                raise
...
class AuthPlugin(object):
    ...
    def apply(self, callback, route):
        def wrapper(*args, **kwargs):
            # all the valid operation is moved to auth module
            # auth module will check the configration data for
            # whether the auth feature enabled. not glad to import
            # settings module in bottle, so move it to auth
            if not auth.valid_token_in_request(request):
                raise AuthError(msg="invalid token or token expired")
...
class SignaturePlugin(object):
    ...
    def apply(self, callback, route):
        def wrapper(*args, **kwargs):
            jstring = callback(*args, **kwargs)
            # tell the ret is a json string
            if jstring[-1] != '}':
                return jstring
            ret = "%s, \"signature\": \"%s\"}" \
                  %(jstring[:-1], signature(jstring))
            return ret
        return wrapper
