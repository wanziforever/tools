...
from mystat import PlayTimeGrabber
from mystat import PlayTimeCalc, PlayTimeCalcMgr
...
START_DESC = '''
 +-------------------------------------------------------------
   Statistical Analysis for PLAY DURATION
...
def calc():
    ...
    for i in range(grabber_num):
        name = 'PlayTimeGrabber' + str(i)
        config = {'start':ts_calc_list[i][0], 'end':ts_calc_list[i][1],
                  'log_start_ts': ts_list[i][0], 'log_end_ts': ts_list[i][1]}
        gmgr.set_worker(name, PlayTimeGrabber, config)

    config = {'start':start_day, 'end':end_day}
    cmgr = PlayTimeCalcMgr(msgh, config)
    for i in range(calc_num):
        name = 'PlayTimeCalc' + str(i)
        cmgr.set_worker(name, PlayTimeCalc, config)
