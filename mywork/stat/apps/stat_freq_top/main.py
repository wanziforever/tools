...
from mystat import AccessFreqNginxGrabber
from mystat import AccessFreqCalc, AccessFreqCalcMgr
from mystat import determine_api, dbname_dict
...
START_DESC = '''
 +-------------------------------------------------------------
   Statistical Analysis for ACCESS FREQ ({3})
...
FINISH_DESC = '''
 +------------------------------------------------------------
   Statistical Analysis for ACCESS FREQ ({5})
   Search criteria is DATE From {0} to {1}
...
def usage():
    print "%s <start_day> <end_day> <type_string>\n" \
          "-- day should be written like 2014-5-1\n"%sys.argv[0]
    print "support type strings are:"
    for tstr in dbname_dict.values():
        print tstr
...
def calc():
    ...
    print START_DESC.format(start_day, end_day, str(program_start_time)[:19], sys.argv[3])
...
if __name__ == "__main__":
    ...
    if not determine_api(sys.argv[3]):
        print "ERROR: fail to determine the api, check the type string: %s"%sys.argv[3]
        usage()
        exit(0)
    calc()
