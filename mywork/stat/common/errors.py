...
class CodedError(Exception):
    def __init__(self, code, msg=None, status_code=None, **kw):
...
class ArgError(CodedError):
    def __init__(self, code, message, param=None, status_code=http.BAD_REQUEST):
        super(ArgError, self).__init__(code, message, status_code, param=param)
        
class ArgMissingError(ArgError):
    def __init__(self, name):
        super(ArgMissingError, self).__init__('ARG_MISSING', '"%s" is required.' % name, name, http.BAD_REQUEST)

class AuthError(CodedError):
    def __init__(self, msg='Authentication failed.'):
        super(AuthError, self).__init__('AUTH_FAILED', msg, http.UNAUTHORIZED)
...
class BadArgTypeError(ArgError):
    def __init__(self, name, actual_type, supported_types):
        ...
        super(BadArgTypeError, self).__init__('INVALID_ARG_TYPE', msg, name, http.BAD_REQUEST)
...
class BadArgValueError(ArgError):
    def __init__(self, name, value=None, msg=None):
        ...
        super(BadArgValueError, self).__init__('INVALID_ARG_VALUE', msg, name, http.BAD_REQUEST)
...
class DuplicatedError(CodedError):
    def __init__(self, resource_type, key):
        ...
        super(DuplicatedError, self).__init__('DUPLICATED_%s' % typename, '%s with key=%s already exists.' % (typename, key), http.CONFLICT)
        
class LoginError(CodedError):
    def __init__(self, msg='Invalid user name or password.'):
        super(LoginError, self).__init__('LOGIN_FAILED', msg, http.UNAUTHORIZED)
        
class PermissionDeniedError(CodedError):
    def __init__(self, type='', key=None, msg=None):
        if msg is None:
            if key is None:
                msg = 'Permission is denied on %s' % type
            else:
                msg = 'Permission is denied on %s %s' % (type, key)
        super(PermissionDeniedError, self).__init__('PERMISSION_DENIED', msg, http.FORBIDDEN)
...
class ResourceConflictError(CodedError):
    def __init__(self, msg='resource conflict'):
        super(ResourceConflictError, self).__init__('RESOURCE_CONFLICT', msg, http.CONFLICT)

        
class ResourceNotFoundError(CodedError):
    def __init__(self, resource_type, key, status_code=http.BAD_REQUEST):
        '''
        if resource_type is string
        ...
        if isinstance(resource_type, type):
            ...
            typename = resource_type.upper()
        super(ResourceNotFoundError, self).__init__('%s_NOT_FOUND' % typename, '%s %s cannot be found.' % (typename, key), status_code=status_code)
...
class PageNotFoundError(ResourceNotFoundError):
    def __init__(self, page):
        super(PageNotFoundError, self).__init__('PAGE', page, http.NOT_FOUND)
   
class ApiNotFoundError(ResourceNotFoundError):
    def __init__(self, api):
        super(ApiNotFoundError, self).__init__('API', api, http.NOT_FOUND)   
     
class UnknownError(CodedError):
    def __init__(self, msg='Unknown error occured.'):
        super(UnknownError, self).__init__('UNKNOWN_ERROR', msg, http.INTERNAL_ERROR)


class HandleNotFoundError(ResourceNotFoundError):
    def __init__(self, handler):
        super(HandleNotFoundError, self).__init__('Data Handler', handler, http.NOT_FOUND)
